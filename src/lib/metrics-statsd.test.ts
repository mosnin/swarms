...
describe("StatsdMetrics", () => {
  function recorder() {
    ...
    const m = new StatsdMetrics({ prefix: "swarms", send: (p) => packets.push(p) });
    return { m, packets };
  }
  ...
  it("formats a counter in DogStatsD format with tags", () => {
    ...
    expect(packets[0]).toBe("swarms.jobs.processed:1|c|#status:succeeded");
  });

  it("formats a timing", () => {
    const { m, packets } = recorder();
    m.timing("job.duration", 1234, { runner: "mock" });
    expect(packets[0]).toBe("swarms.job.duration:1234|ms|#runner:mock");
  });

  it("omits the tag section when there are no tags", () => {
    const { m, packets } = recorder();
    m.increment("x");
    expect(packets[0]).toBe("swarms.x:1|c");
  });
});
